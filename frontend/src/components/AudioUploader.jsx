...
const AudioUploader = ({ onFileSelected, onFileRemoved, authorizeStart }) => {
  ...
  const handleFileChange = (e) => {
    ...
    if (f.size / 1024 / 1024 > maxMB) {
      alert(`File too large — must be under ${maxMB}MB.`);
      return;
    }
  ...
  const removeFile = () => {
    ...
    // ❗ Notify parent to clear analysis
    ...
    <div className="bg-white p-8 rounded-2xl shadow-xl border border-purple-100">
      ...
      {!fileInfo ? (
        <>
          <button
            onClick={chooseFile}
            className="w-full px-6 py-3 rounded-xl text-white font-medium bg-gradient-to-r 
            from-orange-400 to-pink-500 shadow-md hover:opacity-90 transition"
          >
            Choose File
          </button>
